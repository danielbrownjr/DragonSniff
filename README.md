...
The bounded churn runner deliberately repeats the read-only SSE lifecycle against the Dragon address entered in the normal connection card. Starting churn pauses an active normal observation only after its workers and connection permits stop cleanly, then restores observation after churn reaches a terminal state and completes cleanup. Automated Churn and Thermal runs remain mutually exclusive. Churn is sequential only: connect, observe until the duration or event bound, disconnect, verify local cleanup, sample health, wait, and repeat.
